        ...
        <form method="post" action="{{ action('OrganizationController@update', $organization->id) }}">
            ...
            <div class="form-body mxn1">
                ...
                <div class="rel mb2 col col-12 px1 js-field-active">
                    ...
                    <div class="px1 h5">
                        @foreach($tags as $tag)
                            <input type="checkbox" class="tag-checkbox sr-only" id="tag_{{ $tag->id }}" name="tags[]" value="{{ $tag->id }}" {{ in_array($tag->id, old('tags', $organization->tags->lists('id')->toArray())) ? 'checked' : '' }}>
